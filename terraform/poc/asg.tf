...
- action: aws:runShellScript
  ...
  inputs:
    ...
    - ansible-playbook deploy-custodian.yml -e region="${var.region}" -e neo4j_password="${var.database_password}" -e domain="${var.domain_name}" -e stack_name="${var.stack_name}" -e database_asg_name="${var.database_asg_name}" -e cluster_name="${var.ecs_cluster_name}"
  DOC
  tags = merge(
  {
